...
class Movie {
public:
  Movie(const std::string& m) : name{m} {}
...
std::ostream& operator<<(std::ostream& o, const Movie& a) {
  o << a.name;
  return o;
}
...
int main() {
  ...
  Movie a{"scary"};
  movieLibrary.add(a);
  std::cout << std::boolalpha << "confirming that a is in library: " << movieLibrary.inLibrary(a) << std::endl;
