...
bool School::addCourse(const std::string& courseCode) {
	...
	auto courseMatches = [&courseCode] (std::shared_ptr<Course> c) {
	...
	auto result = std::find_if(courses.begin(),courses.end(), courseMatches);
...
void School::addStudentToCourse(unsigned int id, const std::string& courseCode) {
  	...
  	// if the student can't be added to the course, e.g. they are already enrolled then return false.
