...
bool School::Course::addStudent(std::weak_ptr<Student> wp) {
  ...
  auto studentExists = [wp] (std::weak_ptr<Student> s) {
    return wp.lock() == s.lock();
  }; // changed into a separate anon-function
  auto result = std::find_if(studentsInCourse.begin(),studentsInCourse.end(), studentExists);
...
bool School::addCourse(const std::string& courseCode) {
  // double check that the course doesn't already exist.
  auto courseExists = [&courseCode] (std::shared_ptr<Course> c) {
    if (c->getCourseCode().compare(courseCode) == 0) return true;
      return false;
  };
  auto result = std::find_if(courses.begin(), courses.end(), courseExists);
...
void School::addStudentToCourse(unsigned int id, const std::string& courseCode) {
  ...
  //DONE: try to add the student to the course.
