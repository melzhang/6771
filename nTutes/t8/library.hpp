...
public:
  ...
  using iterator = LibraryItemIterator<T, U>;
  friend iterator;
...
template <typename T, typename U>
void Library<T,U>::add(const T& t) {
  const auto find_t = [&t] (const auto& item) {
    return item.getItem() == t;
  };
  if (std::find_if(items.cbegin(), items.cend(), find_t) == items.cend())
...
unsigned int Library<T,U>::remove(const T& t) {
  ...
  items.erase(std::find_if(items.cbegin(), items.cend(), find_t));
...
void Library<T,U>::ItemContainer::RelatedWork::printItemAndDescription() {
  if (auto sp = relatedWorkLink.lock())
